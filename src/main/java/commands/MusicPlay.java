					...
					if(!selfVoiceState.inVoiceChannel()) {
						...
						if(!memberVoiceState.inVoiceChannel()) {
							...
							if(!args[2].startsWith("https://")) {
								...
								LOGGER.info("{} - [{}] did not give a link", event.getAuthor().getAsTag(), Tool.combine(args, 2));
							} else {
								try {
									PlayerManager.getInstance().loadAndPlay(channel, args[2]);
									LOGGER.info("{} - [{}] song: {}", event.getAuthor().getAsTag(), args[1], Tool.combine(args, 2));
								} catch(Exception e) {
									event.getChannel().sendMessage("bruh i cant play that").queue();
									LOGGER.info("{} - [{}] something broke, managed to get to the catch", event.getAuthor().getAsTag(), Tool.combine(args, 2));
								}
							}
						}
					}
